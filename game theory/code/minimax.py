...
class MiniMax(object):
    ...
    def get_next_board(self):
        board = self.minimax(self.board, 0, self.max_pl)
    ...
    def print_log(self, board, depth, evaluation):
        ...
        if depth == 0:
            ...
            self.traverse_log.append('%s%s,%s,%s' % (y_map[board.last_positionY], x_map[board.last_positionX], depth, evaluation))
    ...
    def minimax(self, board, depth, current_pl):
            ...
            for i in range(0, self.board.length):
                for j in range(0, self.board.length):
                    ...
                    if new_board.valid_square(i, j):
                        current_board = self.minimax(new_board, depth + 1, new_board.pl2)
