...
class AlphaBeta(object):
    ...
    def __init__(self, board, max_depth):
        self.board = board
        self.board.set_evaluation(AlphaBeta.int_min)
        self.max_depth = int(max_depth)
        self.max_pl = board.pl1
    ...
    def print_log(self, board, depth, evaluation, alpha, beta):
        ...
        if depth == 0:
            ...
            self.traverse_log.append('%s%s,%s,%s,%s,%s' % (y_map[board.last_positionY], x_map[board.last_positionX], depth, evaluation, alpha, beta))
    ...
    def get_next_board(self):
        board = self.alphabeta(self.board, 0, self.int_min, self.int_max, self.max_pl)
        board.set_pl(self.board.pl1)
        return board

    def alphabeta(self, board, depth, alpha, beta, current_pl):
    # touch the leaf, return board
        if depth == self.max_depth:
            self.print_log(board, depth, board.evaluation, alpha, beta)
            return board
        
        if current_pl != board.pl1:
            board.switch_pl()
    
        if current_pl == self.max_pl:
            ...
            for i in range(0, self.board.length):
                for j in range(0, self.board.length):
                    ...
                    if new_board.valid_square(i, j):
                        current_board = self.alphabeta(new_board, depth + 1, alpha, beta, new_board.pl2)
